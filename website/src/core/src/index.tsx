...
export interface SignatureProps extends React.SVGProps<SVGSVGElement> {
  ...
  readonly?: boolean;
  fill?: string,
  defaultPoints?: { points: Record<string, number[][]>, fill: Record<string, string | undefined> };
  renderPath?: (d: string, keyName: string, point: number[][], index: number, container: SVGElement) => JSX.Element;
  onPointer?: (points: number[][]) => void;
}

const Signature = forwardRef<SignatureRef, SignatureProps>(
  ({ children, options, fill, renderPath, defaultPoints, ...props }, ref) => {
    ...
    return (
      <PointerContext.Provider value={state}>
        <PointerDispatchContext.Provider value={dispatch}>
          <Container {...props} fill={fill} ref={ref}>
