...
export const Signature = forwardRef<SignatureRef, Omit<SignatureProps, 'defaultPoints' | 'renderPath' | 'options'>>(
  (props, ref) => {
    const { className, prefixCls = 'w-signature', style, readonly = false, onPointer, fill, children, ...others } = props;
    ...
    useImperativeHandle<SignatureRef, SignatureRef>(
      ref,
      () => ({ svg: $svg.current, dispatch, clear: () => dispatch({points: {}, fill: {}}) }),
      [dispatch],
    );
    ...
    const handlePointerDown = useEvent((e: React.PointerEvent<SVGSVGElement>) => {
      ...
      dispatch({
        points: {[pointId + pointCount.current]: pointsRef.current},
        fill: {[pointId + pointCount.current]: fill}
      });
    });
